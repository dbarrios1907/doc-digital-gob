<template>
  <div class="dx-notification" :class="'dx-notif' + uuid" :style="{ backgroundColor: backgroundColor, color: textColor }">
    <div class="dx-container">
      <div :class="{ 'dx-icn-left': showLeftIcn }" :style="{ fill: textColor }">
        <svg v-if="showLeftIcn" class="dx-icone" width="14" height="20" viewBox="0 0 14 20" xmlns="http://www.w3.org/2000/svg">
          <path
      ...
      <div>
        <p v-if="title !== null && title !== undefined && title !== ''" class="dx-title">{{ title }}</p>
        <p v-if="messageIsHTML" class="dx-message" v-html="message" />
        <p v-else class="dx-message">{{ message }}</p>
      </div>

      <div :style="{ fill: textColor }" class="dx-closeButton" @click="remove()">
        <svg v-if="showCloseIcn" width="10" height="10" viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
          <path
...
export default {
  ...
  props: {
    ...
    primary: {
      ...
      default() {
        return {
          background: currentColor(this.dark).primary,
          color: currentColor(this.dark).light,
          titleColor: '',
        }
      },
    },
    success: {
      ...
      default() {
        return {
          background: currentColor(this.dark).success,
          color: currentColor(this.dark).light,
        }
      },
    },
    error: {
      ...
      default() {
        return {
          background: currentColor(this.dark).error,
          color: currentColor(this.dark).light,
        }
      },
    },
    warning: {
      ...
      default() {
        return {
          background: currentColor(this.dark).warning,
          color: currentColor(this.dark).light,
        }
      },
    },
    info: {
      ...
      default() {
        return {
          background: currentColor(this.dark).secondary,
          color: currentColor(this.dark).darken3,
        }
      },
    },
    animateIn: {
      ...
      default() {
        const tl = new TimelineMax()
          .from(`.dx-notif${this.uuid}`, 0.3, {
            opacity: 0,
          })
          .from(this.notificationEl, 0.2, {
            borderRadius: 100,
            width: 20,
            height: 20,
          })
          .from(`.dx-notif${this.uuid}>div`, 0.3, {
            opacity: 0,
          })
        tl.pause()
        return tl
    ...
    animateOut: {
      ...
      default() {
        return new TimelineMax({})
          .to(`.dx-notif${this.uuid}>div`, 0.4, {
            opacity: 0,
            ease: Power3.easeOut,
          })
          .to(`.dx-notif${this.uuid}`, 0.2, {
            borderRadius: 100,
            width: 30,
            height: 30,
          })
          .to(`.dx-notif${this.uuid}`, 0.3, {
            borderRadius: 100,
            width: 0,
            height: 0,
            margin: '0 auto',
          })
          .to(`.dx-notif${this.uuid}`, 0.7, {
            opacity: 0,
          })
      },
    },
  ...
  data() {
    return {
      notification: `.dx-notification`,
      notificationEl: `.dx-notif${this.uuid}`,
      notificationElContent: `.dx-notif${this.uuid} .dx-container`,
      notificationTitle: `.dx-notif${this.uuid} .dx-title`,
      notificationMessage: `.dx-notif${this.uuid} .dx-message`,
      notificationIcone: `.dx-notif${this.uuid} .dx-icone`,
      notificationIconeClose: `.dx-notif${this.uuid} .dx-closeButton`,
    }
  },
  beforeCreate() {
  ...
  beforeMount() {
    const notificationContainer = this.getContainer()
    this.initColors()
  ...
  methods: {
    getContainer() {
      // Create the dx-container if they don't exist or return it
